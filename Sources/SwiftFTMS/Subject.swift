...
public class Subject<T> {
    ...
    public func attach(observer: @escaping (T) -> Void) -> Disposable {
        self.attach(observer: observer, updateOnMainThread: false)
    }
    
    public func attach(observer: @escaping (T) -> Void, updateOnMainThread: Bool) -> Disposable {
        let wrapped = ObserverWrapper(subject: self, function: observer, updateOnMainThread: updateOnMainThread)
...
private class ObserverWrapper<T>: Disposable {
    ...
    var updateOnMainThread = false
    
    init(subject: Subject<T>, function: @escaping (T) -> Void, updateOnMainThread: Bool) {
        self.subject = subject
        self.function = function
        self.updateOnMainThread = updateOnMainThread
    }
